...
def location(request, slug):
    ...
    boys_one_plus = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Мальчик", requirement__name='1+'))
    girls_one_plus = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Девочка", requirement__name='1+'))
    boys_one = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Мальчик", requirement__name='1'))
    girls_one = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Девочка", requirement__name='1'))
    boys_two_plus = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Мальчик", requirement__name='2+'))
    girls_two_plus = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Девочка", requirement__name='2+'))
    boys_two = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Мальчик", requirement__name='2'))
    girls_two = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Девочка", requirement__name='2'))
    boys_three_plus = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Мальчик", requirement__name='3+'))
    girls_three_plus = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Девочка", requirement__name='3+'))
    boys_three = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Мальчик", requirement__name='3'))
    girls_three = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Девочка", requirement__name='3'))
    boys = list(Schoolboy.objects.filter(
        classroom__slug=slug, gender__name="Мальчик", requirement__name='-'))
    girls = list(Schoolboy.objects.filter(
...
def location_save(request, slug):
    now = datetime.now()
    try:
        location_last = Location.objects.filter(classroom__slug=slug, save_bd=True).latest('create_data')
